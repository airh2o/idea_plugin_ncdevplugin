...
import com.puppycrawl.tools.checkstyle.api.AbstarctCheckImpl;
...
public class ScanFiles implements Callable<Map<PsiFile, List<Problem>>> {
    ...
    public final Map<PsiFile, List<Problem>> call() {
        try {
            AbstarctCheckImpl.initProjectSourceMap(project);
    ...
    private Map<PsiFile, List<Problem>> checkFiles(final Module module,
        ...
        try {
            ...
            return configurationLocations.stream()
                    .map(configurationLocation -> checkerFactory().checker(module, configurationLocation)
