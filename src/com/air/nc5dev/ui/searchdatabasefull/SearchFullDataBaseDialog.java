...
public class SearchFullDataBaseDialog extends DialogWrapper {
        ...
        JLabel label_6;
        {
            ...
            comboBox_dataSourceIndex.addActionListener(e -> {
                ...
                if (dataSourceVO.getDatabaseType().toLowerCase().contains("oracle")) {
                    textField_selectAllTableSql.setText(" select * from (select lower(table_name) name from " +
                            "user_tables) where name not like '%temp%' and name not like '%log%' ");
                } else if (dataSourceVO.getDatabaseType().toLowerCase().contains("server")) {
                    textField_selectAllTableSql.setText(" select name from sys.objects where type='U' and name not " +
                            "like '%temp%' and name not like '%log%' ");
    ...
    public void initDefualtValues() {
        textField_selectAllTableSql.setText(" select * from (select lower(table_name) name from user_tables) where " +
                "name not like '%temp%' and name not like '%log%' ");
    ...
    public void onOK() {
        ...
        backgroundable.setCancelText("停止搜素");
        backgroundable.setCancelTooltipText("停止搜索");
