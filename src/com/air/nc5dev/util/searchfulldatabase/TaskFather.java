...
public class TaskFather {
    ...
    public void start() {
        ...
        try {
            ...
            conn = dbConnectionFactory.getConnection(config.getDataSource(), config.indicator);
            ...
            while (rs.next() && !config.indicator.isCanceled()) {
                tabs.add(rs.getString(1));
            }
            if (tabs.size() < 1 || config.indicator.isCanceled()) {
