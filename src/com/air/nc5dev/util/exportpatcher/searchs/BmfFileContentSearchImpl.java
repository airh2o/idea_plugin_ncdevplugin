 ...
 */
@Data
public class BmfFileContentSearchImpl extends AbstractContentSearchImpl {
    @Override
        ...
        while (moduleIterator.hasNext() && !contentVO.indicator.isCanceled()) {
         ...
         //   compilerModuleExtension = CompilerModuleExtension.getInstance(module.getModule());
            ...
            if (contentVO.exportModuleMetadata && bmfDir.isDirectory() && !contentVO.indicator.isCanceled()) {
                contentVO.indicator.setText("导出模块元数据:" + bmfDir.getPath());
                contentVO.addOutFiles(new FileContentVO()
                        .setModule(module)
                        .setFile(bmfDir.getPath())
                        .setFileTo(new File(contentVO.outPath + File.separatorChar + module.getConfigName()
                                + File.separatorChar + bmfDir.getName()).getPath())
                        .setName(FileContentVO.NAME_MODULE_BMF)
                );
            }
